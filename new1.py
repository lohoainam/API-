...
def get_proxy():
    try:
        response = requests.get('https://gimmeproxy.com/api/getProxy', verify=False)
...
def display_proxy_info(proxy_info):
    if proxy_info:
        ...
        # Hiển thị thông tin proxy trên một dòng
        proxy_display = f"IP: {ip}, Port: {port}, Country: {country}, Protocol: {protocol}, Anonymity Level: {anonymity_level}"
        print(proxy_display)
