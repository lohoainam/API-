...
# Tùy chỉnh yêu cầu API
api_url = 'https://gimmeproxy.com/api/getProxy?anonymityLevel=1'
...
def display_proxy_info(proxy_info):
    if proxy_info:
        ...
        # In thông tin proxy trên một dòng
        proxy_info_str = f"IP:{ip}, Port:{port}, Country:{country},Level:{anonymity_level}"
        print(proxy_info_str)
        return f"{ip}:{port}"
    return None
